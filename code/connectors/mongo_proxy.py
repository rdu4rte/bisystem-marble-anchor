...
class Executable(object):
    ...
    def __init__(
        self, method, logger, wait_time=None, disconnect_on_timeout=True
    ):
        self.method = method
        self.logger = logger
            ...
            try:
                ...
                if delta >= max_time:
                    ...
                    if conn:
                        ...
                        i = 0
                        self.logger.warning(
                            "Resetting clock for round 2 "
                            "after disconnecting"
                        )
                self.logger.warning(
                    "AutoReconnecting, "
                    "try %d.%d (%.1f seconds elapsed)" % (round, i, delta)
                )
...
class MongoProxy(object):
    ...
    def __init__(
        self, conn, logger=None, wait_time=None, disconnect_on_timeout=True
    ):
    ...
    def __getattr__(self, key):
        """If key is the name of an executable method in the MongoDB conn,
